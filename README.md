...
| `font-size`     | 10 pt body text       | dominant char size on body pages ≥ 9.8 pt                                          | ❌ error (⚠️ < 9.9)               |
| `font-family`   | Times Roman           | dominant font is a Times variant                                                   | ❌ error                          |
| `leading`       | 12 pt leading         | dominant baseline distance ≥ 11.8 pt, per body page                                | ❌ error (⚠️ < 11.9)              |
| `two-column`    | two-column format     | ≥ 60 % of lines in one half-page                                                   | ❌ error                          |
| `heading-space` | no whitespace removal | ≥ 2 headings with < 10.5 pt above (template ~13.7 pt)                              | ❌ error                          |
| `title-space`   | no whitespace removal | whitespace below the page-1 title ≥ 20 pt (template ~30 pt)                        | ❌ error (⚠️ < 24)                |
| `front-matter`  | front matter not squeezed | page-1 body (abstract) starts ≥ 2.8" down (template reserves ~3.5"); above that, the fixed title area was reclaimed (an empty author block is fine) | ❌ error                          |
| `anonymity`     | anonymous submission  | author block has an "Anonymous…"-style marker; names, affiliations, e-mail flagged | ❌ error                          |
...
| `metadata`      | anonymous submission  | no author name in PDF metadata (a blinded field, e.g. "Anonymous", is fine)        | ❌ error                          |
